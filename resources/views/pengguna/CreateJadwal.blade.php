...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white shadow-sm rounded-lg p-6">
        <form action="{{ route('user/pengguna/StoreJadwal') }}" method="POST">
            @csrf

            <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-8">
                <div class="mb-6 hidden">
                    <label for="name" class="block text-sm font-medium text-gray-700">Nama</label>
                    <input id="name" name="name" type="text" value="{{ auth()->user()->name }}" readonly class="form-input mt-1 block w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                </div>
                
                <div class="mb-6 hidden">
                    <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                    <input id="email" name="email" type="email" value="{{ auth()->user()->email }}" readonly class="form-input mt-1 block w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                </div>                
